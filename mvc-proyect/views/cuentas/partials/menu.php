    ...
    <div class="container-fluid">
        ...
        <div class="collapse navbar-collapse" id="navbarSupportedContent">
            <ul class="navbar-nav me-auto mb-2 mb-lg-0">
                <li class="nav-item">
                    <a class="nav-link <?= (in_array($_SESSION['id_rol'], $GLOBALS['cuentas']['new'])) ? null : 'disabled' ?>"
                        aria-current="page" href="<?= URL ?>cuentas/nuevo">Nuevo</a>
                </li>
                <!-- Agregar opción para exportar CSV -->
                <li class="nav-item">
                    <a class="nav-link <?= (in_array($_SESSION['id_rol'], $GLOBALS['cuentas']['exportar'])) ? null : 'disabled' ?>"
                        href="
                        <?= URL ?>cuentas/exportar">Exportar CSV</a>
                </li>
                <!-- Agregar opción para importar CSV -->
                <li class="nav-item">
                    <button type="button"
                        class="nav-link btn btn-link <?= (in_array($_SESSION['id_rol'], $GLOBALS['cuentas']['importar'])) ? null : 'disabled' ?>"
                        data-bs-toggle="modal" data-bs-target="#importarModal">Importar CSV</button>
                </li>
                <!-- PDF -->
                <li class="nav-item">
                    <a class="nav-link <?= (in_array($_SESSION['id_rol'], $GLOBALS['cuentas']['pdf']) || in_array($_SESSION['id_rol'], $GLOBALS['cuentas']['pdf'])) ? 'active' : 'disabled' ?>"
                        aria-current="page" href="<?= URL ?>cuentas/pdf">PDF</a>
                </li>
                <li class="nav-item dropdown">
                    <a class="nav-link <?= in_array($_SESSION['id_rol'], $GLOBALS['cuentas']['order']) ? null : 'disabled' ?> dropdown-toggle"
